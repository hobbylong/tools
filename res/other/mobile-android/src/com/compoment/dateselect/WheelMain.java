...
//		int day = calendar.get(Calendar.DATE);
		...
		OnWheelChangedListener wheelListener_month = new OnWheelChangedListener() {
			public void onChanged(WheelView wheel, int oldValue, int newValue) {
				int dayIndex = wv_day.getCurrentItem();
				String year = (String) wv_year.getAdapter().getItem(wv_year.getCurrentItem());
				int month_num = newValue + 1;
				Log.i("Tang", "month_num:"+month_num);
				// 判断大小月及是否闰年,用来确定"日"的数据
				if (list_big.contains(String.valueOf(month_num))) {//一月31天的月份
					wv_day.setAdapter(new NumericWheelAdapter(1, 31));
				} else if (list_little.contains(String.valueOf(month_num))) {//一月30天的月份
					wv_day.setAdapter(new NumericWheelAdapter(1, 30));
					if (dayIndex > 29) {
