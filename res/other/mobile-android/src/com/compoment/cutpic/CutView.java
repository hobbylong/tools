...
public class CutView extends View
{
    ...
    * */
	...
	protected void onDraw(Canvas canvas)
	{
		...
		if(m_bmp != null)
		{
			if(m_matrix == null)
			{
				...
				m_matrix.postScale(m_sx, m_sy, m_x + m_bmp.getWidth()/2, m_y + m_bmp.getHeight()/2);  //���š�����
			...
			m_matrix.mapPoints(temp_dst, temp_src);  //ӳ��������ĵ�
			...
			canvas.drawRect(0, 0, getWidth(), (int)temp_dst2[1], temp_paint);  //��
			canvas.drawRect(0, (int)temp_dst2[1], (int)temp_dst2[0], (int)temp_dst2[3], temp_paint);  //��
			canvas.drawRect((int)temp_dst2[2], (int)temp_dst2[1], getWidth(), (int)temp_dst2[3], temp_paint);  //��
			canvas.drawRect(0, (int)temp_dst2[3], getWidth(), getHeight(), temp_paint); //��
	...
	private float w;
	private float h;
	private final float MIN_LEN = 0.08f;
		...
		switch(event.getPointerCount())
		{
			case 1: //��ָ
				...
				case MotionEvent.ACTION_DOWN:
					temp_src4[0] = m_rect.m_x*m_bmp.getWidth();  //��¼�ĸ���
					...
					m_matrix.mapPoints(temp_dst4, temp_src4); //ӳ����������ĸ���
					
					temp_matrix.reset();
					m_matrix.invert(temp_matrix); //�������
					temp_src[0] = event.getX();
					temp_src[1] = event.getY();
					temp_matrix.mapPoints(temp_dst,temp_src);  //ӳ�䵽ͼƬ�ϵ����
...
//						Log.i("move", "dist_x = "+dist_x);
	...
	public void setScale(float s) //���þ��εĿ�߱�
	{
		if(m_r % 180 !=0)
		{
			m_scale = 1.0f/s;
...
//		m_sy = scale;
	...
	public ImgBean GetCutImage()
	{
		if(m_bmp != null){
			...
			Matrix m = new Matrix();
			m.postTranslate(-x, -y);   //�ƶ���ԭ��
			m.postRotate(m_r, w/2f, h/2f);  //��������ת
			
			if(m_sx < 0 || m_sy < 0)
			{
				m.postScale(m_sx, m_sy, w/2f, h/2f);  //����
				if(m_r % 180 != 0)
				{
			...
			if(m_r % 180 != 0)
			{
				m.postTranslate(h/2f - w/2f, w/2f - h/2f); //�������ת180�ȣ���ת���ƻ�ԭ��
			}
