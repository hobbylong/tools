...
public class TbQuestionSugguestAction {
	...
	private TbQuestionSugguestBean entity;
	...
	public TbQuestionSugguestBean getEntity() {
		return entity;
	}

	public void setEntity(TbQuestionSugguestBean tbquestionsugguest) {
...
e.printStackTrace();
 ...
 //第三步  正确跳转到哪  错误跳转到哪  一般用forward
	...
	public String query(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			tbquestionsugguestBeans = tbQuestionSugguestService.get(reqMap, false);
		} catch (Exception e) {
			e.printStackTrace();
		}
		int total = 0;
		try {
			total = tbQuestionSugguestService.getCount(reqMap);
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		jsonObject.put("resultSize", tbquestionsugguestBeans.size());
		jsonObject.put("resultTotal", total);
...
isOk=false;
  ...
  try {
  	StrutsParamUtils.getResponse().getWriter().write("{"returnCode":"00","info":"成功。","returnData""}");
  }catch(
