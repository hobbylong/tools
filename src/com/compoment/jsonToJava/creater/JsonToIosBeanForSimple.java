...
public class JsonToIosBeanForSimple {
	...
	public static void main(String[] args) {
		JsonToIosBeanForSimple pojo = new JsonToIosBeanForSimple();
		String jsonString = pojo.jsonFromFile();
		JSONObject jsonObject = JSONObject.fromObject(jsonString);
		Bean rootBean = pojo.relation("root", jsonObject,null);
		pojo.createJavaBeanClass(rootBean);
		System.out.println(pojo.getJaveBeanClass());
	
	}

	public JsonToIosBeanForSimple() {

	}

	public JsonToIosBeanForSimple(String jsonString) {
		JSONObject jsonObject = JSONObject.fromObject(jsonString);
		Bean rootBean = relation("root", jsonObject,null);
	...
	public void createJavaBeanClass(Bean rootBean) {
		...
		while (it.hasNext()) {
			...
			n+="NSString *"+key+"=["+rootBean.name+" objectForKey:@\""+key+"\"];\n";
		...
		for (Bean bean : rootBean.childs) {
			...
			n+="NSDictionary *"+bean.name+"=["+rootBean.name+" objectForKey:@\""+bean.name+"\"];\n";
	...
	public Bean relation(String name, JSONObject jsonObject,Bean parent) {
		Bean bean = new Bean();
		bean.name = name;
		bean.parent=parent;
		...
		try {
			...
			while (it.hasNext()) {
				...
				if (obj instanceof JSONArray) {
					...
					Bean child = relation(key, (JSONObject) obj,bean);
